...
export function StoreCard({ store }: StoreCardProps) {
  return (
    <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm ring-1 ring-slate-900/5 transition hover:shadow-md lg:rounded-xl lg:p-4">
      <div className="flex gap-3 p-3 md:gap-4 md:p-4 lg:gap-5 lg:flex-col">
  ...
  />
        ...
        <div className="flex min-w-0 flex-1 flex-col justify-between md:gap-2">
          <div className="space-y-1">
            <h3 className="line-clamp-1 text-base font-extrabold text-slate-900 md:text-lg">{store.name}</h3>
            <p className="text-xs font-semibold text-slate-500 md:text-sm">{store.category}</p>
          ...
          <div className="mt-2 flex items-center justify-between gap-2 md:mt-3 md:gap-3 lg:flex-col lg:items-start">
            <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-semibold text-slate-600 md:text-xs">{store.floor}</span>
            <Link
              href={`/store/${store.id}`}
